...
public class Laser {
    ...
    public static void addLaser(Player p) {
        Profile profile = Core.getInstance().getProfileManager().getProfile(p);
        int cost = getUpgradeCost(profile.getData().getPrisonLaserLevel().getAmount());

        if(profile.getData().getPrisonLaserLevel().getAmount() == 10) {
